...
- Security: absolute filesystem paths no longer leaked in any server-facing output; `dependencyHandlers.ts` uses `basename()` in McpError messages and project-name report fields, `configValidator.ts` uses `basename()` in all user-facing messages, and `findPackageFiles` uses `getRelativePath()` for scan-error messages (#129)
