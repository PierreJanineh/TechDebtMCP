...
### Fixed
- **Claude Code plugin layout** (TEC-242) — two release-blocking bugs in the marketplace install path, fixed in a single restructure. (1) `commands/` and `skills/` were nested under `.claude-plugin/`, which the CC loader does not scan, so `/techdebt-scan`, `/techdebt-file`, `/techdebt-summary`, and the `proactive-analysis` skill never loaded for any installer. (2) The marketplace install with `source: "./"` performed a filesystem copy of the entire repo (~193 MB including `node_modules/`, `.claude/`, `src/`, `tests/`, `docs/`, `.idea/`, `.env`) into every user's plugin cache; the `plugin.json` schema defines no `files`/`include`/`ignore`/`exclude` field, so there is no first-class way to restrict the install surface. **Fix:** moved the entire plugin install surface into a `plugin/` subdirectory (manifest, slash commands, `proactive-analysis` skill, README) and changed `marketplace.json` `source` to `./plugin` — only that subtree is now copied to user caches. `src/server/__tests__/pluginManifest.test.ts` gained three regression guards: the marketplace-source allowlist, the plugin-root component layout check, and a surface allowlist that fails CI if anything other than `{.claude-plugin/, commands/, skills/, README.md}` appears under `plugin/`. Surfaced by the v2.1.0 sanity-run (#238).
- **`analyze_project` returned 0 issues when `languages`/`categories` were empty arrays** (TEC-73, #240) — MCP Inspector's auto-generated forms default array fields to `[]`, which caused `analyze_project` to filter to an empty language/category set and report "Health Score: 100/100, 0 issues" for any repository. `optionalLanguages` / `optionalCategories` in `src/server/inputParser.ts` now coerce empty arrays to `undefined`, so the three call shapes `{}`, `{ languages: undefined, categories: undefined }`, and `{ languages: [], categories: [] }` produce identical output. Surfaced by the v2.1.0 sanity-run (#238).
- **`execute_custom_rules` infers language from file extension** (TEC-50) — when called with `path` and no explicit `language`, the handler now derives the language via `detectLanguageFromExtension(path)` before invoking the engine, so rules carrying a `languages` filter actually fire. Previously such rules were silently skipped on the path entry point while still firing for `code`/`language` calls.
