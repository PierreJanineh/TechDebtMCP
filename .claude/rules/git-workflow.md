...
- **`develop` is the default branch and the trunk.** All work branches from `develop`.
...
- Releases: cut `release/vX.X.X` from `develop` → merge fix/doc PRs into the release branch → tag `vX.X.X` on the release branch → GitHub Actions validates tag matches `package.json` version → publishes to npm via OIDC → back-merge `release/vX.X.X` → `develop`. Tag and `package.json` version must match exactly. There is no `master` branch; `develop` is the single long-lived trunk.
