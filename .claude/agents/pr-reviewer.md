...
**Token setup** — the bot token is pre-generated by the parent session and passed via the `TECHDEBT_GH_TOKEN` environment variable or directly in the prompt. Save it to a temp file for convenience:

```bash
echo "$TECHDEBT_GH_TOKEN" > /tmp/techdebt_gh_token
```

If the token was passed inline in the prompt (not as an env var), write it directly:
...
**Note:** Tokens expire after ~10 minutes. If you get a 401 error mid-workflow, the parent session needs to regenerate — stop and report this.
