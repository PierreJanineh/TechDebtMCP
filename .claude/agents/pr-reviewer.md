...
**Token setup** — the bot token is passed directly in the prompt as a literal value like `ghs_abc123...`. Use it **inline** in every GitHub API call — never via echo, temp files, or subshells:

```bash
GH_TOKEN=ghs_TOKEN_VALUE_HERE gh api repos/PierreJanineh/TechDebtMCP/...
```

Replace `ghs_TOKEN_VALUE_HERE` with the exact token string from your prompt each time.
...
```bash
GH_TOKEN=ghs_TOKEN_VALUE_HERE gh api user --jq '.login' 2>&1
```
...
```bash
GH_TOKEN=ghs_TOKEN_VALUE_HERE gh api repos/PierreJanineh/TechDebtMCP/pulls/<number>/comments \
  -f body='<your reply>' \
...
```bash
GH_TOKEN=ghs_TOKEN_VALUE_HERE gh api graphql -f query='mutation {
