...
```bash
GH_TOKEN=ghs_TOKEN_VALUE_HERE gh pr create ...
GH_TOKEN=ghs_TOKEN_VALUE_HERE gh api graphql ...
```
...
```bash
GH_TOKEN=ghs_TOKEN_VALUE_HERE gh api graphql -f query='mutation { resolveReviewThread(input: {threadId: "<id>"}) { thread { isResolved } } }'
```
