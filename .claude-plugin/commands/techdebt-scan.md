...
allowed-tools: ["mcp__tech-debt-mcp__analyze_project", "mcp__tech-debt-mcp__get_debt_summary", "mcp__tech-debt-mcp__get_recommendations", "mcp__tech-debt-mcp__get_sqale_metrics"]
---
...
4. Call `mcp__tech-debt-mcp__get_recommendations` with `{ "path": "<resolved-path>" }`.

5. Present a concise summary:
   - Health score and SQALE rating.
   - Total issue count broken down by severity (critical / high / medium / low).
   - Estimated total remediation time.
   - Top 5 prioritized recommendations with file, line, effort, and a one-line fix hint.

6. Offer to dive deeper into any specific file, category, or severity if the user wants more detail.
