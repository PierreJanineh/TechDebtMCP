...
Tech Debt MCP is a Model Context Protocol (MCP) server that performs static analysis of source code on your local machine. This page is a rendered mirror of [`PRIVACY.md`](https://github.com/PierreJanineh/TechDebtMCP/blob/develop/PRIVACY.md), the canonical policy in the repository.
...
**None in the server.** The server emits no usage events, crash reports, or performance metrics to any remote endpoint.

**This documentation site** uses a self-hosted, cookieless [Plausible](https://plausible.io/) instance for aggregate page-view counts. No identifying information is collected, no cookies are set, and the analytics instance is operated by the project maintainer (not a third-party SaaS). This applies only to visitors of the documentation site; it does not apply to the MCP server itself.
...
Material changes to this policy will be announced in [`CHANGELOG.md`](https://github.com/PierreJanineh/TechDebtMCP/blob/develop/CHANGELOG.md) and reflected in the **Last updated** date above. Historical versions are available via the [git history of `PRIVACY.md`](https://github.com/PierreJanineh/TechDebtMCP/commits/develop/PRIVACY.md).

## Contact

Questions about this policy: open a [GitHub Discussion](https://github.com/PierreJanineh/TechDebtMCP/discussions) or email the maintainer at **janinehpierre@gmail.com**.

For security-sensitive reports, use [GitHub Security Advisories](https://github.com/PierreJanineh/TechDebtMCP/security/advisories/new) instead — see the [Security Model](/security) page for the disclosure policy.
