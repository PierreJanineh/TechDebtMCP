...
for (const repo of manifest.repos) {
  ...
  const cachedPath = await cloneAtSha(owner, name, sha);

  // Copy the cached clone to a temporary directory before scanning so that
  // config-file removal (below) does not mutate the cached working tree.
  // A dirty cache would cause cloneAtSha() to re-clone on every subsequent
  // run, defeating the docs.yml cache and adding network flakiness.
  const scanPath = join(tmpdir(), `td-showcase-${owner}-${name}-${sha.slice(0, 12)}-${Date.now()}`);
  await cp(cachedPath, scanPath, { recursive: true });
  ...
  try {
    ...
    // We remove from the temp copy, leaving the cached clone untouched.
    for (const cfgName of ['.techdebtrc.json', '.techdebtrc', 'techdebt.config.json', '.techdebt.json']) {
      ...
      if (existsSync(cfgPath)) {
        ...
        console.log(`[scan-showcase] removed ${cfgName} from ${owner}/${name} scan copy (cache untouched)`);
      }
    }
    ...
    // Always remove the temp scan copy so the cached clone remains clean
    // and reusable by subsequent runs without triggering a re-clone.
